...
const Navbar = () => {
    ...
    const sendVerificationOtp = async ()=>{
      if (await send(backendUrl + '/api/auth/send-verify-otp')) navigate('/email-verify')
    }
      ...
      {userData ? (
        <div className='w-8 h-8 flex justify-center items-center rounded-full bg-black text-white relative group cursor-pointer'>
          ...
          <div className='absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded pt-10'>
            <ul className='list-none m-0 p-2 bg-gray-100 text-sm whitespace-nowrap'>
              {!userData.isAccountVerified &&
                <li><button disabled={sending || cooldown > 0} onClick={sendVerificationOtp} className='py-1 px-2 hover:bg-gray-200 disabled:opacity-50'>{sending ? 'Sending…' : 'Verify email'}</button></li>}
